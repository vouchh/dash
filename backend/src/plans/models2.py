...
class Plan(models.Model):
    ...
    available = models.BooleanField(default=False, help_text=_('Is still available for purchase'))
    visible = models.BooleanField(default=True, help_text=_('Is visible in current offer'))
...
class AccountPlan(models.Model):
    account = models.OneToOneField(Account, on_delete=models.CASCADE)
    plan = models.ForeignKey(on_delete=models.CASCADE)
    ...
    def get_plan_extended_from(self, plan):
        if plan.is_free():
            return None
        if not self.is_expired() and self.expires_at is not None and self.plan == plan:
            return self.expire
        return date.today()
...
class RecurringAccounPlan(models.Model):
    account_plan = models.OneToOneField('AccountPlan', on_delete=models.CASCADE)
    token = models.CharField(max_length=255, default=None, blank=True, )
    payment_provider = models.CharField(max_length=255, default=None, blank=True)
    pricing = models.ForeignKey('Pricing', default=None, null=True, blank=True, on_delete=models.CASCADE)
    amount = models.DecimalField(max_digits=7, decimal_places=2, null=True, blank=True)
    currency = models.CharField(max_length=3)
    has_automatic_renewal = models.BooleanField(default=False)
    ...
    def create_renew_order(self):
        ...
        return Order.objects.create(
            ...
            pricing=accountplan.recurring.pricing,
            amount=accountplan.recurring.amount,
            tax=accountplan.recurring.tax,
            currency=accountplan.recurring.currency,
        )
    """
